<script>
  ...
  function qualityTooltip(tunnel) {
    const parts = [];
    parts.push(`Latency: ${tunnel.latencyMs}ms`);
    parts.push(`Jitter: ${tunnel.jitterMs}ms`);
    parts.push(`Packet loss: ${formatLoss(tunnel.packetLoss)}`);
    if (tunnel.packetLoss >= 0.10) {
      parts.push('High packet loss — connection unstable');
    } else if (tunnel.jitterMs >= 80) {
      parts.push('High jitter — expect lag spikes');
    } else if (tunnel.packetLoss >= 0.03) {
      parts.push('Moderate packet loss');
    } else if (tunnel.jitterMs >= 30) {
      parts.push('Elevated jitter');
    }
    return parts.join('\n');
  }
    ...
    {#each tunnels as tunnel}
      <div class="flex items-center gap-1.5 px-2 py-0.5 rounded text-xs shrink-0">
        ...
        {#if tunnel.state === 'up'}
          ...
          <!-- Packet loss -->
          <span class="font-mono tabular-nums {lossColor(tunnel.packetLoss)}" title={qualityTooltip(tunnel)}>
            {formatLoss(tunnel.packetLoss)}
          </span>

          <!-- Warning icon -->
          {#if hasWarning(tunnel)}
            <span class="{warningColor(tunnel)}" title={qualityTooltip(tunnel)}>
